...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteCommodities(id) {
            if(window.confirm("您确定要删除货物"+name+"吗？")) {
                $("#dFruitId").val(id);
                // 引入分页信息至该 form 表单
...
<body onload="init()">
    ...
    <div class="addMask">
        <div class="c">
            ...
            <form action="/commodities/add.action" method="post" id="addForm" onsubmit="return checkAddCommodities()">
                货物名：<input type="text" id="addName" name="name" style="width: 120px;"/><br/>
    ...
    <div class="mask">
        <div class="c">
            ...
            <form action="/commodities/edit.action" method="post" id="editForm">
                货物名称：<input type="text" id="editName" name="name" style="width: 120px;"/><br/>
    ...
    <form action="/commodities/list.action" id="listForm" method="post">
        ...
        <input type="submit" value="搜索" style="background-color: #27c3ff; color: #FFFFFF; width: 70px"/><br/>
        <!-- 显示错误信息 -->
    ...
    <c:if test="${list!=null}">
        <table style="margin-top: 10px; width: 700px; text-align: center;" border="1">
            <tr>
                <td>序号</td><td>货物名</td><td>价格</td>
                <td>产地</td><td>创建日期</td><td>操作</td>
            </tr>
            <c:forEach items="${list}" var="item" varStatus="status">
                <tr>
                    ...
                    <td>
                        <a onclick="editCommodities(('${item.fruitId}'))">编辑</a> | <a onclick="deleteCommodities('${item.fruitId}', '${item.name}')">删除</a> | <a onclick="openwin('${item.fruitId}')">附属品</a>
                        <form action="/commodities/delete.action" id="deleteForm" method="post">
